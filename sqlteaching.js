...
var levels = [{'name': 'SELECT *',
              ...
              {'name': 'Joins',
               ...
               'prompt': 'Previous lessons focused on single tables.<br/><br/> But different parts of information can be stored in different tables, and in order to put them together, we use <code>JOIN ... ON</code>. Joining tables gets to the core of SQL functionality, but it can get very complicated. We will start with a simple example.<br/><br/>As you can see below, there are 3 tables:<br/><span id="table-name">character</span>: Each character is a row and is represented by a unique identifier (<i>id</i>), e.g. 1 is Doogie Howser<br/><span id="table-name">character_tv_show:</span> For each character, which show is he/she in?<br/><span id="table-name">character_actor</span>: For each character, who is the actor?<br/><br/>See that <span id="table-name">character_tv_show</span>, instead of storing both the character and TV show names (e.g. Willow Rosenberg and Buffy the Vampire Slayer), stores the <i>character_id</i> as a substitue for the character name. (This is commonly done to save memory space.) This <i>character_id</i> refers to the matching <i>id</i> row from the <span id="table-name">character</span> table.<br/><br/>This allows us to "join" the tables together "on" that reference/common column. <br/><br/>To get each character name with his/her TV show name, we can write <br/><code>SELECT *<br/> FROM character <br/>JOIN character_tv_show<br/> ON character.id = character_tv_show.character_id;</code><br/>This puts together every row in <span id="table-name">character</span> with the corresponding row in <span id="table-name">character_tv_show</span>, or vice versa.<br/><br/>Notice:<br/>- We use the syntax <span id="table-name">table_name</span>.<i>column_name</i>. If we only used <i>column_name</i>, SQL might incorrectly assume which table it is coming from.<br/>- <code>SELECT *</code> selects all columns from both tables.<br/><br/>Can you use a join to pair each  character name with the actor who plays them? (Select all columns.)'}
              ];

...
var levels = [{'name': 'SELECT *',
              ...
              {'name': 'MAX and MIN',
               'short_name': 'max_min',
               'database_type': 'family_and_legs',
               'answer': {'columns': ['id', 'name', 'gender', 'species', 'salary', 'num_legs', 'MAX(salary)'],
                          'values': [[1, 'Dave', 'male', 'human', 60000, 2, 60000]]},
               'prompt': 'We can use the <code>MAX</code> and <code>MIN</code> to find the maximum or minimum value of a table.  In addition to getting the value, you can also get the row associated with the value by adding additional columns (or * to get all columns). <br/><br/>To find the family member with the most legs, you can run <br/><code>SELECT *, MAX(num_legs) FROM family_members;</code> <br/><br/>Can you find the family member with the highest salary?'},
